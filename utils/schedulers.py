...
#=== End of Poly_Warm_Cos_LR


# CosineDecay Scheduler
class CosineDecayLR(_LRScheduler):
    # modified version of LambdaLR from https://pytorch.org/docs/stable/_modules/torch/optim/lr_scheduler.html#LambdaLR
    # cosine 반주기 (0 ~ pi) 형태로 initial LR 에서 eta_min으로 도달하는 LRScheduler
    # BSRN에서 사용됨
    
    def __init__(self, optimizer, last_epoch=-1, verbose=False, eta_min=1e-7, max_iter=5000):
        self.optimizer = optimizer
        self.eta_min   = eta_min      # 최소 LR (마지막 LR과 동일)
        self.max_iter  = max_iter     # 최대 update 횟수
        
        lr_lambda = lambda x : (1 + math.cos(math.pi*(x/max_iter)))
    ...
    def get_lr(self):
        out_lr_raw = [self.eta_min + 0.5*(base_lr-self.eta_min)*lmbda(self.last_epoch)
        ...
        try:    # complex 클래스 발생 문제 해결목적
            if out_lr < self.eta_min:
                warnings.warn("(CosineDecayLR) lr fixed to eta_min")
                out_lr = self.eta_min
        except:
            warnings.warn("(CosineDecayLR) lr generated as complex value")
            warnings.warn("(CosineDecayLR) lr fixed to eta_min")
            out_lr = self.eta_min
        
        return [out_lr]

#=== End of CosineDecayLR